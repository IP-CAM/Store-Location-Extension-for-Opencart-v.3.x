...
class ControllerAccountRegister extends Controller {
    ...
    public function cnpj() {
        ...
        if (isset($this->request->get['cnpj'])) {
...
//Criando Comunicação cURL
        ...
        //curl_setopt($ch, CURLOPT_URL, "http://feasso.objectdata.com.br/cliente/cadastro_juridica/consulta_receitaws?cpf_cnpj=" . $cnpj);
...
//curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); //Descomente esta linha apenas se você não usar HTTPS, ou se estiver testando localmente
        ...
        // $this->response->addHeader('Content-Type: application/json');
        // $this->response->setOutput(json_encode($retorno));
    }
    ...
    public function check() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            /* if (!isset($this->request->post['cnpj'])) {
              $json['error'] = $this->language->get('Já existe um CNPJ cadastrado no sistema');
              } elseif (!$this->model_account_customer->getTotalCustomersByCNPJ($this->request->post['cnpj'])) {
              $json['error'] = $this->language->get('Já existe um CNPJ cadastrado no sistema');
              } */

            if (!$this->model_account_customer->getTotalCustomersByCNPJ($this->request->post['cnpj'])) {
                $json['error'] = $this->language->get('Já existe um CNPJ cadastrado no sistema');
            }
