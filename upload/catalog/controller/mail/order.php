...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		if($this->session->data['invoice_no'] == ''){
                $pedido = $order_info['order_id'];
		}else{
		 $pedido = $this->session->data['invoice_no'];	
		}
                
		$data['title'] = sprintf($language->get('text_subject'), $order_info['store_name'],$pedido );
		...
		$mail->setSubject(html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $pedido), ENT_QUOTES, 'UTF-8'));
		$mail->setHtml($this->load->view('mail/order_add', $data));
		$mail->send();
                unset($this->session->data['invoice_no']);
	}
