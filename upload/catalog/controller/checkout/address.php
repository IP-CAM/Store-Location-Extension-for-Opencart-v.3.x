...
class ControllerCheckoutAddress extends Controller{
	...
	$data['cnpj'] = preg_replace("/[^0-9]/", "",json_decode($customer_info['custom_field'], true)[$this->config->get('module_feasso_cadastro_cnpj')]);// CAMPO PERSONALIZADO COD. CLIENTE
        $data['firstname'] = $this->customer->getFirstName();
        
	$this->response->setOutput($this->load->view('checkout/address', $data));
    ...
    public function saveAddress($src)    {
        ...
        if ($this->customer->isLogged()) {
            ...
            if (!empty($address_data['country_id'])) {
                $country = $this->model_localisation_country->getCountry($address_data['country_id']);
            ...
            if (!empty($address_data['zone_id'])) {
                $zone = $this->model_localisation_zone->getZone($address_data['zone_id']);
        ...
        $custom_fields = $this->model_account_custom_field->getCustomFields($this->config->get('config_customer_group_id'));
				...
				foreach ($custom_fields as $custom_field) {
					if ($custom_field['location'] == 'address') {
						if ($custom_field['required'] && empty($this->request->post[$prefix.'_custom_field'][$custom_field['location']][$custom_field['custom_field_id']])) {
						...
						} elseif (($custom_field['type'] == 'text') && !empty($custom_field['validation']) && !filter_var($this->request->post[$prefix.'_custom_field'][$custom_field['location']][$custom_field['custom_field_id']], FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => $custom_field['validation'])))) {
							$json['error'][$prefix.'_custom_field' . $custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field'), $custom_field['name']);
						}
					}
				}
