<?php
class ControllerCommonCartMobile extends Controller {
	public function index() {
		...
		}elseif ($this->cart->getFornecedor() > 14) {
					$data['error_warning'] = " É necessário que compre de 1 fornecedor por vez, devido a política de vendas ser diferentes entre os mesmos.";
		}elseif (($this->cart->getFornecedor() == 11) AND ($this->cart->getSubTotal() < $this->cart->getMinimofeasso())) {
					$data['error_warning'] = " É necessário que complete o pedido mínimo de ".$this->cart->getMinimofeasso()." reais.";
		}elseif (($this->cart->getFornecedor() == 12) AND ($this->cart->getSubTotal() < 100)) {
					$data['error_warning'] = " É necessário que complete o pedido mínimo da RRL que é 100 reais.";
		...
		return $this->load->view('common/mobile/cart', $data);
	}
